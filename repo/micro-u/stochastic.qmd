...
## Settings
::: {.callout-note title="Dfn: Binary Random Choice Rule"}
...
::: {.callout-note title="Dfn: Random Choice Rule"}
...
::: {.callout-tip title="Example 1" collapse="true"}
Given $X = \{x,y,z\}$ and $A = \{x,y\}$, 
$$\begin{aligned}
...
::: {.callout-warning title="Intuition"}
The possibility of choosing an option cannot become greater when new choices emerge. This implies that every $\mu \in [0,1]$ and $\sum \mu = 1$.
:::  

::: {.callout-tip title="Example 2" collapse="true"}
Given $X = \{x,y,z,w\}$ and $A = \{x,y\}$.
$$\begin{aligned}
      q(x, A) &= \sum_{B \supseteq A} (-1)^{|B \setminus A|} \rho(x,B) \\
...
::: {.callout-warning title="Intuition"}
If we use $\mu$ in random preference model to denote the results above:  
